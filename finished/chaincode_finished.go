...
var addUserStr = "_addUser"
var editUserStr = "_editUser"
var getUserStr = "_getUser"
var getAttachmentStr = "_getAttachment"
var getLogStr = "_getLog"
...
type HashAttachments struct {
	HaId        string       `json:ha_id`
	Attachments []Attachment `json:attachments`
}

type AllHashAttachments struct {
	HashAttachments []HashAttachments `json:hash_attachments`
}
...
func (t *SimpleChaincode) Invoke(stub shim.ChaincodeStubInterface, function string, args []string) ([]byte, error) {
	...
	if function == "init" { //initialize the chaincode state, used as reset
		...
		return t.addEmail(stub, args[0], args[1])
	} else if function == "add_attachment" {
		//return t.addObject(stub, args[0], args[1], args[2], args[3])
		return t.addAttachment(stub, args[0], args[1], args[2]);
...
func (t *SimpleChaincode) Query(stub shim.ChaincodeStubInterface, function string, args []string) ([]byte, error) {
	...
	if function == "read" { //read a variable
		...
		return t.getEmailsListFromAttachment(stub, args[0])
	}else if function == "getEmailsListOfUser" {
		return t.getEmailsListOfUser(stub, args[0])
	}
...
func (t *SimpleChaincode) addObject(stub shim.ChaincodeStubInterface, objId string, objType string, content string, userId string) ([]byte, error) {
	...
	err = stub.PutState(objIndexStr, jsonAsBytes) //store name of email
	if err != nil {
		return nil, err
	}
...
func (t *SimpleChaincode) addEmail(stub shim.ChaincodeStubInterface, userid string, data string) ([]byte, error) {
	var err error

	//get the email index
	emailAsBytes, err := stub.GetState(userid)
	if err != nil {
		return nil, errors.New("Failed to get email index")
	}
	...
	fmt.Println("! email index: ", emailIndex)
	jsonAsBytes, _ := json.Marshal(emailIndex)
	err = stub.PutState(userid, jsonAsBytes) //store name of email
	if err != nil {
		return nil, err
	}

	fmt.Println("- end add email")
	valAsbytes, err := stub.GetState(userid) //get the var from chaincode state
	if err != nil {
		jsonResp := "{\"Error\":\"Failed to get state for " + objIndexStr + "\"}"
		return nil, errors.New(jsonResp)
	}
...
func (t *SimpleChaincode) addAttachment(stub shim.ChaincodeStubInterface, fileHash string, userid string, emailContent string) ([]byte, error) {
	var err error

	//get the email index
	
	emailAsBytes, err := stub.GetState(fileHash)
	//userEmailAsBytes, err2 := stub.GetState(userid)
	if err != nil {
		return nil, errors.New("Failed to get email index")
	}
	var emailIndex []string
	//var useremailIndex []string
	json.Unmarshal(emailAsBytes, &emailIndex) //un stringify it aka JSON.parse()
	//json.Unmarshal(userEmailAsBytes, &useremailIndex) //un stringify it aka JSON.parse()
	//append
	//currentTime := time.Now()
	//email := Obj{ObjId: objId, ObjType: objType, Content: content, UserId: userId, CrtDt: currentTime.String()}
	email := emailContent;
	...
	fmt.Println("! email index: ", emailIndex)
	jsonAsBytes, _ := json.Marshal(emailIndex)
	err = stub.PutState(fileHash, jsonAsBytes) //store hash+userid/email pair
	if err != nil {
		return nil, err
	}

	//useremailIndex = append(useremailIndex, email)
	//userjsonAsBytes, _ := json.Marshal(useremailIndex)
	//err = stub.PutState(userid, userjsonAsBytes) //store userid/mail pair
	//if err != nil {
	//	return nil, err
	//}
...
func (t *SimpleChaincode) addHashAttachment(stub shim.ChaincodeStubInterface, haId string, userId string, objId string) ([]byte, error) {
	...
	var allHa AllHashAttachments
	var allHaIndex []HashAttachments
	json.Unmarshal(allHaAsBytes, &allHa) //un stringify it aka JSON.parse()
	...
	allHa.HashAttachments = allHaIndex
	fmt.Println("! hash attachment index: ", allHa)
	jsonAsBytes, _ := json.Marshal(allHa)
	err = stub.PutState(allHaStr, jsonAsBytes) //store name of email
	if err != nil {
		return nil, err
	}
...
func (t *SimpleChaincode) getEmailsListOfUser(stub shim.ChaincodeStubInterface, userid string) ([]byte, error) {
	var err error
